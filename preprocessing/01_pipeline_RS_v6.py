...
class PreprocessingRS(object):
    ...
    def _clean_signals(self, sps, fmriname="fmri"):
        ...
        if "pnm" in self.denoising_regs:
            ...
            os.system(f'cp {physiopath}/{subname}_evlist.txt {sps}/{out_reg}.txt;')
            # transform nii to matrices per slice
            nuisance_mat_dict['pnm'] = self.__create_confounds_pnm(sps,f'{out_reg}.txt')
        ...
        ## transform the PNM regressors from imgs (nii) to confounds matrix that will be used
        ## per slice 
        
        if "pnm" in self.denoising_regs or "csf" in self.denoising_regs:
            # create confounds per slice
            nuisance_pnm_ps = self.__create_confounds_pnm(sps,f'{out_reg}.txt')

        if "pnm" in self.denoising_regs:
            nuisance_mat_dict['pnm'] = nuisance_pnm_ps
        elif "csf" in self.denoising_regs and "pnm" not in self.denoising_regs:
            nuisance_mat_dict['csfonly'] = nuisance_pnm_ps
        elif "csf" in self.denoising_regs and "pnm" in self.denoising_regs:
            add_outname += "_csf"

        ## All the others are volume regressors so we need to simply repeat them per slice

        if "moco" in self.denoising_regs:
            print("### Info: adding moco params regressors ...")
            # edit moco files in txt
            moco_file = os.path.join(sps, f"Nuisance/{moco}_nohdr.txt")
            ...
            moco_mat = np.loadtxt(moco_file)
            nuisance_mat_dict['moco'] = np.stack([moco_mat] * self.nslices, axis=0)
            add_outname += "_moco"
        ...
        if "compcor" in self.denoising_regs:
            ...
            compcor_file = os.path.join(sps,f"Nuisance/{compcorname}_nohdr.txt")
            cc_mat = np.loadtxt(compcor_file)
            nuisance_mat_dict['compcor'] = np.stack([cc_mat] * self.nslices, axis=0)
            add_outname += "_cc"
        # Combination of denoising regressors iterating over the dictionary

        # count the shape of the regressors to allocate nuisance matrix
        nregressors = 0
        nuisance_mat_ls = []
        for k,v in nuisance_mat_dict.items():
            if v is not None:
                print(f"### Info: using regressors {k}...")
                nregressors += v.shape[-1]
                nuisance_mat_ls.append(v)

        ## nuisance matrix has a dimensionality nslices, tpoints, nregressors
        nuisance_mat = np.concatenate(nuisance_mat_ls, axis=-1)
        print(f"### Info: shape of nuisance matrix is {nuisance_mat.shape}")    
        ...
        # but we are applying it slice-wise so first split the images
        ...
        img_cleaned_full = np.zeros(self.fmri_data.shape)
        for i in range(self.nslices):
            slice_confounds = nuisance_mat[i,:,:]
            img_cleaned = clean_img(os.path.join(sps, f'splitted_mfmri{add}/m{self.fmriname}_{i}.nii.gz'),confounds=slice_confounds,
                                    low_pass=high_cutoff, high_pass=low_cutoff,t_r=self.TR, standardize=self.standardize_ts)
            img_cleaned_data = img_cleaned.get_fdata()
            img_cleaned_full[:,:,i,:] = np.squeeze(img_cleaned_data, axis=2)
        ...
        img_nii = nib.Nifti1Image(img_cleaned_full,self.fmri_affine, self.fmri_header)
        # save cleaned img
        nib.save(img_nii,os.path.join(sps, denoised_name))
